...
export const problemsModule = new Elysia({ prefix: "/v1/problems" })
  ...
  .get(
    "/:id",
    async ({ params: { id }, set }) => {
      const problemList = await db.select().from(problems).where(eq(problems.id, id)).limit(1);
      ...
      const approaches = await db.select().from(solutionApproaches).where(eq(solutionApproaches.problemId, id));
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      const newProblem = {
        ...
        solutionApproachIds: [],
        difficulty: body.difficulty || { estimatedLevel: 1, confidence: 0.5, calibrated: false },
        sourceIds: body.sourceIds || [],
        status: body.status || "draft",
      };

      await db.insert(problems).values(newProblem);
      const created = (await db.select().from(problems).where(eq(problems.id, id)).limit(1))[0];
